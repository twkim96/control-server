...
test("managed home rejects broad destructive targets", () => {
  ...
  assert.equal(isWithin("/tmp/control", "/tmp/control/releases/1.5.1"), true);
...
test("health gate waits for the expected release version", async () => {
  ...
  globalThis.fetch = async () => {
    calls += 1;
    return new Response(
      JSON.stringify({ ok: true, version: calls === 1 ? "1.5.0" : "1.5.1" }),
      { status: 200, headers: { "content-type": "application/json" } },
    );
  };
  try {
    assert.equal(await waitForHealth("http://127.0.0.1:9000", 2_000, "1.5.1"), true);
...
test("no-start install creates one release and preserves private data on uninstall", async () => {
  ...
  try {
    ...
    assert.equal(installed.metadata.version, "1.5.1");
    ...
    assert.equal(inspection.metadata.version, "1.5.1");
...
test("failed update restores current release and metadata", async () => {
  ...
  const home = path.join(parent, "managed");
  const source = path.join(parent, "source-1.5.2");
  const previousSkip = process.env.CONTROL_SERVER_TEST_SKIP_DEPS;
  const previousLabel = process.env.CONTROL_LAUNCHD_LABEL;
  process.env.CONTROL_SERVER_TEST_SKIP_DEPS = "1";
  ...
  try {
    ...
    const layout = resolveLayout(home);
    assert.equal(fs.readlinkSync(layout.current), path.join("releases", "1.5.1"));
    assert.equal(JSON.parse(await fs.promises.readFile(layout.metadataPath, "utf8")).version, "1.5.1");
    assert.equal(fs.existsSync(path.join(layout.releases, "1.5.2")), true);
...
test("explicit rollback selects a completed older release", async () => {
  ...
  try {
    ...
    assert.equal(result.toVersion, "1.5.1");
    const layout = resolveLayout(home);
    assert.equal(fs.readlinkSync(layout.current), path.join("releases", "1.5.1"));
    assert.equal(
