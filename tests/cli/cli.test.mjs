...
test("managed home rejects broad destructive targets", () => {
  ...
  assert.equal(isWithin("/tmp/control", "/tmp/control/releases/1.5.2"), true);
...
test("health gate waits for the expected release version", async () => {
  ...
  globalThis.fetch = async () => {
    calls += 1;
    return new Response(
      JSON.stringify({ ok: true, version: calls === 1 ? "1.5.1" : "1.5.2" }),
      { status: 200, headers: { "content-type": "application/json" } },
    );
  };
  try {
    assert.equal(await waitForHealth("http://127.0.0.1:9000", 2_000, "1.5.2"), true);
